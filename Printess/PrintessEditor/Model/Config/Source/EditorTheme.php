...
/**
 * Used as both an EAV source model (product attribute) and a system-config source model.
 */
class EditorTheme extends AbstractSource implements OptionSourceInterface
{
    ...
    private function fetchOptions(): array
    {
        ...
        try {
            ...
            $response = $api->readUserSettings(['bcui-all-themes']);
            $raw      = $response->{'bcui-all-themes'} ?? '[]';
            $list     = is_string($raw) ? (json_decode($raw, true) ?? []) : (array)$raw;

            foreach ($list as $theme) {
                // API returns objects: {"name":"ThemeName",...}
                $name = is_array($theme) ? (string)($theme['name'] ?? '') : (string)$theme;
                if ($name === '') {
                    continue;
                }
